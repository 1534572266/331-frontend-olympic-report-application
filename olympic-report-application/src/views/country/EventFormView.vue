...
import EventService from '@/services/CountryService';
  ...
  EventService.saveEvent(event.value)
    .then((response) => {
      ...
      store.updateMessage('You are successfully add a new event for ' +
        response.data.title, 'detailView')
      setTimeout(() => {
        store.resetMessage()
      }, 3000)
    })
...
<template>
  <div>
    ...
    <form @submit.prevent="saveEvent">
      ...
      <h3>Who is Organizing Your Event?</h3>
      <label>Select an Organizer</label>
      <BaseSelect v-model="event.organizer.id" :options="organizers" label="Organizer" />
      <h3>Pets Allowed?</h3>
      <div class="field-group">
        <BaseInput v-model="event.petsAllowed" type="text" label="PetsAllowed" />
      </div>
